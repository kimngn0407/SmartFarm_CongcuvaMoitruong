  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Lấy tất cả field của tất cả farm
        ...
        // Lấy trạng thái từng field qua API /api/fields/{id}
        await Promise.all(allFields.map(async (field) => {
          try {
            ...
            // Đếm cảm biến và cảnh báo như cũ
            const sensorsResponse = await sensorService.getSensorsByField(field.id);
            totalSensors += sensorsResponse.data.length;
